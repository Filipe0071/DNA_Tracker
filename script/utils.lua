...
function Utils:DrawTextTemplate(
  text,
  x,
  y,
  font,
...
function Utils.PointOnSphere(alt, azu, radius, orgX, orgY, orgZ)
  ...
  alt, azu, radius, orgX, orgY, orgZ =
      (tonumber(alt or 0) or 0) * toradians,
      (tonumber(azu or 0) or 0) * toradians,
      tonumber(radius or 0) or 0,
      tonumber(orgX or 0) or 0,
      tonumber(orgY or 0) or 0,
      tonumber(orgZ or 0) or 0
  if vector3 then
      return vector3(
...
function Utils:FindNearestZone(position, table)
  if type(table) ~= "table" or type(position) ~= "vector3" then
      return 999999
  end
...
function Utils.GetCoordsInFrontOfCam(...)
  local function Distance(v1, v2)
      if not v1 or not v1.x then
          return 0
      end
      v2 = v2 or vector3(0, 0, 0)
      return math.sqrt(
          ((v1.x or 0) - (v2.x or 0)) * ((v1.x or 0) - (v2.x or 0)) +
              ((v1.y or 0) - (v2.y or 0)) * ((v1.y or 0) - (v2.y or 0)) +
              ((v1.z or 0) - (v2.z or 0)) * ((v1.z or 0) - (v2.z or 0))
  ...
  local direction =
      vector3(
      (math.sin(rot.z * (3.141593 / 180)) * -1) * math.abs(math.cos(rot.x)),
      math.cos(rot.z * (3.141593 / 180)) * math.abs(math.cos(rot.x)),
      math.sin(rot.x * (3.141593 / 180))
  )
  local distanceMod = Distance((coords - GetEntityCoords(PlayerPedId(), false) or vector3(0, 0, 0)))
  local retTable = {}
  if (select("#", ...) == 0) then
      return vector3(coords.x + (1 * direction.x), coords.y + (1 * direction.y), coords.z + (1 * direction.z))
  end
  for k = 1, select("#", ...) do
...
function Utils.InRange(val, target, range)
  if target + range > val and target - range < val then
      return true
  else
      return false
  end
...
function Utils:GetKeyPressed(key)
  ...
  if (IsDisabledControlJustPressed(0, Keys[key]) or IsControlJustPressed(0, Keys[key])) then
      return true
  else
      return false
  end
...
function Utils:GetKeyHeld(key)
  ...
  if (IsDisabledControlPressed(0, Keys[key]) or IsControlPressed(0, Keys[key])) then
      return true
  else
      return false
  end
